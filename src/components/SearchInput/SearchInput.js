          ...
          {this.props.showCitySuggestionFlag && (
            <Col lg="6">
              <ul className={classes.SearchInputSuggestion}>
                {(this.props.errorCity || this.props.errorCityMsg) && (
                  <li className={classes.SuggestionItem}>
                    {this.props.errorCityMsg}
                  </li>
                )}
                {this.props.filteredSuggestions.map((city) => (
                  <li
                    className={classes.SuggestionItem}
                    onClick={() =>
                      this.getForeCastHandler(city.woeid, city.title)
                    }
                    key={city.woeid}
                  >
                    {city.title}
